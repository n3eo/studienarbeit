-- ---
-- Globals
-- ---

-- SET SQL_MODE="NO_AUTO_VALUE_ON_ZERO";
-- SET FOREIGN_KEY_CHECKS=0;

-- ---
-- Remove Tables
DROP TABLE IF EXISTS `BildWortZuord`;
DROP TABLE IF EXISTS `VideoWortZuord`;
DROP TABLE IF EXISTS `EbookTransZuord`;
DROP TABLE IF EXISTS `HoerbuchTransZuord`;
DROP TABLE IF EXISTS `VideoTransZuord`;
DROP TABLE IF EXISTS `BildTransZuord`;
DROP TABLE IF EXISTS `BuchTransZuord`;
DROP TABLE IF EXISTS `AutorBuchZuord`;
DROP TABLE IF EXISTS `BuchWortZuord`;

DROP TABLE IF EXISTS `Ausleihe`;
DROP TABLE IF EXISTS `Schlagwort`;

DROP TABLE IF EXISTS `Video`;
DROP TABLE IF EXISTS `Bild`;

DROP TABLE IF EXISTS `Ebooks`;
DROP TABLE IF EXISTS `Hoerbuch`;
DROP TABLE IF EXISTS `Buch`;

DROP TABLE IF EXISTS `Sorte`;
DROP TABLE IF EXISTS `Verlag`;

DROP TABLE IF EXISTS `Maler`;
DROP TABLE IF EXISTS `Ausleiher`;
DROP TABLE IF EXISTS `Sprecher`;
DROP TABLE IF EXISTS `Autor`;
DROP TABLE IF EXISTS `Person`;
-- ---

-- ---
-- Table 'Buch'
-- 
-- ---

CREATE TABLE `Buch` (
  `ISBN` VARCHAR(13) NOT NULL,
  `Titel` VARCHAR(50) NOT NULL,
  `Untertitel` VARCHAR(100) NULL DEFAULT NULL,
  `VerlagId` INTEGER(11) NULL DEFAULT NULL,
  `Erscheinungsjahr` YEAR(4) NULL DEFAULT NULL,
  `SorteId` INTEGER(11) NULL DEFAULT NULL,
  `Kurzbeschreibung` VARCHAR(100) NULL DEFAULT NULL,
  `Preis` DECIMAL(10,3) NULL DEFAULT NULL,
  `Auflage` VARCHAR(20) NULL DEFAULT NULL,
  `Sprache` ENUM("deutsch","englisch","französisch","spanisch","griechisch") NULL DEFAULT NULL,
  `Titelbild` BLOB NULL DEFAULT NULL,
  PRIMARY KEY (`ISBN`)
);

-- ---
-- Table 'Person'
-- 
-- ---
		
CREATE TABLE `Person` (
  `PersonenId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `Vorname` VARCHAR(20) NOT NULL,
  `Name` VARCHAR(30) NOT NULL,
  `Email` VARCHAR(30) NULL DEFAULT NULL,
  `Geburtsdatum` DATE NULL DEFAULT NULL,
  PRIMARY KEY (`PersonenId`)
);

-- ---
-- Table 'Ausleihe'
-- 
-- ---
		
CREATE TABLE `Ausleihe` (
  `TransaktionsId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `AusleiherId` INTEGER(11) NOT NULL,
  `Ausleihdatum` TIMESTAMP NOT NULL DEFAULT NOW(),
  `Rückgabedatum` DATE NULL DEFAULT NULL,
  PRIMARY KEY (`TransaktionsId`)
);

-- ---
-- Table 'AutorBuchZuord'
-- 
-- ---
		
CREATE TABLE `AutorBuchZuord` (
  `AutorId` INTEGER(11) NOT NULL,
  `ISBN` VARCHAR(13) NOT NULL
);

-- ---
-- Table 'BuchWortZuord'
-- 
-- ---
		
CREATE TABLE `BuchWortZuord` (
  `ISBN` VARCHAR(13) NOT NULL,
  `Wort` VARCHAR(20) NOT NULL
);

-- ---
-- Table 'Autor'
-- 
-- ---
		
CREATE TABLE `Autor` (
  `AutorId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `PersonenId` INTEGER(11) NOT NULL,
  `Beschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  PRIMARY KEY (`AutorId`)
);

-- ---
-- Table 'Ausleiher'
-- 
-- ---
		
CREATE TABLE `Ausleiher` (
  `AusleiherId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `PersonenId` INTEGER(11) NOT NULL,
  `Strasse` VARCHAR(30) NOT NULL DEFAULT 'NULL',
  `Postleitzahl` VARCHAR(5) NOT NULL DEFAULT 'NULL',
  `Ort` VARCHAR(30) NOT NULL,
  `Telefonnummer` VARCHAR(15) NULL DEFAULT NULL,
  PRIMARY KEY (`AusleiherId`)
);

-- ---
-- Table 'Schlagwort'
-- 
-- ---
		
CREATE TABLE `Schlagwort` (
  `Wort` VARCHAR(20) NOT NULL,
  `Beschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  PRIMARY KEY (`Wort`)
);

-- ---
-- Table 'Sorte'
-- 
-- ---
		
CREATE TABLE `Sorte` (
  `SorteId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `Name` VARCHAR(20) NULL DEFAULT NULL,
  `Beschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  PRIMARY KEY (`SorteId`)
);

-- ---
-- Table 'Verlag'
-- 
-- ---
		
CREATE TABLE `Verlag` (
  `VerlagId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `Kurzname` VARCHAR(20) NULL DEFAULT NULL,
  `Name` VARCHAR(30) NOT NULL,
  `Postleitzahl` VARCHAR(5) NULL DEFAULT NULL,
  `Strasse` VARCHAR(30) NULL DEFAULT NULL,
  `Internetadresse` VARCHAR(30) NULL DEFAULT NULL,
  `Beschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  PRIMARY KEY (`VerlagId`)
);

-- ---
-- Table 'Ebooks'
-- 
-- ---
		
CREATE TABLE `Ebooks` (
  `ISBN` VARCHAR(13) NOT NULL,
  `BuchISBN` VARCHAR(13) NOT NULL,
  `Dateiformat` VARCHAR(10) NULL DEFAULT NULL,
  PRIMARY KEY (`ISBN`)
);

-- ---
-- Table 'Sprecher'
-- 
-- ---

CREATE TABLE `Sprecher` (
  `SprecherId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `PersonenId` INTEGER(11) NOT NULL,
  `Beschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  PRIMARY KEY (`SprecherId`)
);

-- ---
-- Table 'Video'
-- 
-- ---
		
CREATE TABLE `Video` (
  `VideoId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `Titel` VARCHAR(30) NOT NULL,
  `Untertitel` VARCHAR(60) NULL DEFAULT NULL,
  `Erscheinungsjahr` YEAR(4) NULL DEFAULT NULL,
  `Kurzbeschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  `Sprache` ENUM("deutsch","englisch","französisch","spanisch","griechisch") NULL DEFAULT NULL,
  `SorteId` INTEGER(11) NULL DEFAULT NULL,
  PRIMARY KEY (`VideoId`)
);

-- ---
-- Table 'Hoerbuch'
-- 
-- ---
		
CREATE TABLE `Hoerbuch` (
  `ISBN` VARCHAR(13) NOT NULL,
  `BuchISBN` VARCHAR(13) NOT NULL,
  `SprecherId` INTEGER(11) NOT NULL,
  `VerlagId` INTEGER(11) NULL,
  PRIMARY KEY (`ISBN`)
);

-- ---
-- Table 'Bild'
-- 
-- ---
		
CREATE TABLE `Bild` (
  `BildId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `Titel` VARCHAR(30) NOT NULL,
  `Untertitel` VARCHAR(60) NULL DEFAULT NULL,
  `Erscheinungsjahr` YEAR(4) NULL DEFAULT NULL,
  `Kurzbeschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  `SorteId` INTEGER(11) NULL DEFAULT NULL,
  `MalerId` INTEGER(11) NOT NULL,
  PRIMARY KEY (`BildId`)
);

-- ---
-- Table 'BildWortZuord'
-- 
-- ---
		
CREATE TABLE `BildWortZuord` (
  `BildId` INTEGER(11) NOT NULL,
  `Wort` VARCHAR(20) NOT NULL
);

-- ---
-- Table 'VideoWortZuord'
-- 
-- ---
		
CREATE TABLE `VideoWortZuord` (
  `VideoId` INTEGER(11) NOT NULL,
  `Wort` VARCHAR(20) NOT NULL
);

-- ---
-- Table 'EbookTransZuord'
-- 
-- ---

CREATE TABLE `EbookTransZuord` (
  `ISBN` VARCHAR(13) NOT NULL,
  `TransaktionId` INTEGER(11) NOT NULL
);

-- ---
-- Table 'HoerbuchTransZuord'
-- 
-- ---
		
CREATE TABLE `HoerbuchTransZuord` (
  `ISBN` VARCHAR(13) NOT NULL,
  `TansaktionsId` INTEGER(11) NOT NULL
);

-- ---
-- Table 'VideoTransZuord'
-- 
-- ---
		
CREATE TABLE `VideoTransZuord` (
  `VideoId` INTEGER(11) NOT NULL,
  `TransaktionsId` INTEGER(11) NOT NULL
);

-- ---
-- Table 'BildTransZuord'
-- 
-- ---
		
CREATE TABLE `BildTransZuord` (
  `BildId` INTEGER(11) NOT NULL,
  `TransaktionsId` INTEGER(11) NOT NULL
);

-- ---
-- Table 'BuchTransZuord'
-- 
-- ---

CREATE TABLE `BuchTransZuord` (
  `ISBN` VARCHAR(13) NOT NULL,
  `TransaktionsId` INTEGER(11) NOT NULL
);

-- ---
-- Table 'Maler'
-- 
-- ---
		
CREATE TABLE `Maler` (
  `MalerId` INTEGER(11) NOT NULL AUTO_INCREMENT,
  `PersonenId` INTEGER(11) NOT NULL,
  `Beschreibung` MEDIUMTEXT NULL DEFAULT NULL,
  PRIMARY KEY (`MalerId`)
);

-- ---
-- Foreign Keys 
-- ---

ALTER TABLE `Buch` ADD FOREIGN KEY (VerlagId) REFERENCES `Verlag` (`VerlagId`);
ALTER TABLE `Buch` ADD FOREIGN KEY (SorteId) REFERENCES `Sorte` (`SorteId`);
ALTER TABLE `Ausleihe` ADD FOREIGN KEY (AusleiherId) REFERENCES `Ausleiher` (`AusleiherId`);
ALTER TABLE `AutorBuchZuord` ADD FOREIGN KEY (AutorId) REFERENCES `Autor` (`AutorId`);
ALTER TABLE `AutorBuchZuord` ADD FOREIGN KEY (ISBN) REFERENCES `Buch` (`ISBN`);
ALTER TABLE `BuchWortZuord` ADD FOREIGN KEY (ISBN) REFERENCES `Buch` (`ISBN`);
ALTER TABLE `BuchWortZuord` ADD FOREIGN KEY (Wort) REFERENCES `Schlagwort` (`Wort`);
ALTER TABLE `Autor` ADD FOREIGN KEY (PersonenId) REFERENCES `Person` (`PersonenId`);
ALTER TABLE `Ausleiher` ADD FOREIGN KEY (PersonenId) REFERENCES `Person` (`PersonenId`);
ALTER TABLE `Ebooks` ADD FOREIGN KEY (BuchISBN) REFERENCES `Buch` (`ISBN`);
ALTER TABLE `Sprecher` ADD FOREIGN KEY (PersonenId) REFERENCES `Person` (`PersonenId`);
ALTER TABLE `Video` ADD FOREIGN KEY (SorteId) REFERENCES `Sorte` (`SorteId`);
ALTER TABLE `Hoerbuch` ADD FOREIGN KEY (BuchISBN) REFERENCES `Buch` (`ISBN`);
ALTER TABLE `Hoerbuch` ADD FOREIGN KEY (SprecherId) REFERENCES `Sprecher` (`SprecherId`);
ALTER TABLE `Hoerbuch` ADD FOREIGN KEY (VerlagId) REFERENCES `Verlag` (`VerlagId`);
ALTER TABLE `Bild` ADD FOREIGN KEY (SorteId) REFERENCES `Sorte` (`SorteId`);
ALTER TABLE `Bild` ADD FOREIGN KEY (MalerId) REFERENCES `Maler` (`MalerId`);
ALTER TABLE `BildWortZuord` ADD FOREIGN KEY (BildId) REFERENCES `Bild` (`BildId`);
ALTER TABLE `BildWortZuord` ADD FOREIGN KEY (Wort) REFERENCES `Schlagwort` (`Wort`);
ALTER TABLE `VideoWortZuord` ADD FOREIGN KEY (VideoId) REFERENCES `Video` (`VideoId`);
ALTER TABLE `VideoWortZuord` ADD FOREIGN KEY (Wort) REFERENCES `Schlagwort` (`Wort`);
ALTER TABLE `EbookTransZuord` ADD FOREIGN KEY (ISBN) REFERENCES `Ebooks` (`ISBN`);
ALTER TABLE `EbookTransZuord` ADD FOREIGN KEY (TransaktionId) REFERENCES `Ausleihe` (`TransaktionsId`);
ALTER TABLE `HoerbuchTransZuord` ADD FOREIGN KEY (ISBN) REFERENCES `Hoerbuch` (`ISBN`);
ALTER TABLE `HoerbuchTransZuord` ADD FOREIGN KEY (TansaktionsId) REFERENCES `Ausleihe` (`TransaktionsId`);
ALTER TABLE `VideoTransZuord` ADD FOREIGN KEY (VideoId) REFERENCES `Video` (`VideoId`);
ALTER TABLE `VideoTransZuord` ADD FOREIGN KEY (TransaktionsId) REFERENCES `Ausleihe` (`TransaktionsId`);
ALTER TABLE `BildTransZuord` ADD FOREIGN KEY (BildId) REFERENCES `Bild` (`BildId`);
ALTER TABLE `BildTransZuord` ADD FOREIGN KEY (TransaktionsId) REFERENCES `Ausleihe` (`TransaktionsId`);
ALTER TABLE `BuchTransZuord` ADD FOREIGN KEY (ISBN) REFERENCES `Buch` (`ISBN`);
ALTER TABLE `BuchTransZuord` ADD FOREIGN KEY (TransaktionsId) REFERENCES `Ausleihe` (`TransaktionsId`);
ALTER TABLE `Maler` ADD FOREIGN KEY (PersonenId) REFERENCES `Person` (`PersonenId`);

-- ---
-- Table Properties
-- ---

-- ALTER TABLE `Buch` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Person` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Ausleihe` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `AutorBuchZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `BuchWortZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Autor` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Ausleiher` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Schlagwort` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Sorte` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Verlag` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Ebooks` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Sprecher` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Video` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Hoerbuch` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Bild` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `BildWortZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `VideoWortZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `EbookTransZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `HoerbuchTransZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `VideoTransZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `BildTransZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `BuchTransZuord` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
-- ALTER TABLE `Maler` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;

-- ---
-- Test Data
-- ---

-- INSERT INTO `Buch` (`ISBN`,`Titel`,`Untertitel`,`VerlagId`,`Erscheinungsjahr`,`SorteId`,`Kurzbeschreibung`,`Preis`,`Auflage`,`Sprache`,`Titelbild`) VALUES
-- ('','','','','','','','','','','');
-- INSERT INTO `Person` (`PersonenId`,`Vorname`,`Name`,`Email`,`Geburtsdatum`) VALUES
-- ('','','','','');
-- INSERT INTO `Ausleihe` (`TransaktionsId`,`AusleiherId`,`Ausleihdatum`,`Rückgabedatum`) VALUES
-- ('','','','');
-- INSERT INTO `AutorBuchZuord` (`AutorId`,`ISBN`) VALUES
-- ('','');
-- INSERT INTO `BuchWortZuord` (`ISBN`,`Wort`) VALUES
-- ('','');
-- INSERT INTO `Autor` (`AutorId`,`PersonenId`,`Beschreibung`) VALUES
-- ('','','');
-- INSERT INTO `Ausleiher` (`AusleiherId`,`PersonenId`,`Strasse`,`Postleitzahl`,`Ort`,`Telefonnummer`) VALUES
-- ('','','','','','');
-- INSERT INTO `Schlagwort` (`Wort`,`Beschreibung`) VALUES
-- ('','');
-- INSERT INTO `Sorte` (`SorteId`,`Name`,`Beschreibung`) VALUES
-- ('','','');
-- INSERT INTO `Verlag` (`VerlagId`,`Kurzname`,`Name`,`Postleitzahl`,`Strasse`,`Internetadresse`,`Beschreibung`) VALUES
-- ('','','','','','','');
-- INSERT INTO `Ebooks` (`ISBN`,`BuchISBN`,`Dateiformat`) VALUES
-- ('','','');
-- INSERT INTO `Sprecher` (`SprecherId`,`PersonenId`,`Beschreibung`) VALUES
-- ('','','');
-- INSERT INTO `Video` (`VideoId`,`Titel`,`Untertitel`,`Erscheinungsjahr`,`Kurzbeschreibung`,`Sprache`,`SorteId`) VALUES
-- ('','','','','','','');
-- INSERT INTO `Hoerbuch` (`ISBN`,`BuchISBN`,`SprecherId`,`VerlagId`) VALUES
-- ('','','','');
-- INSERT INTO `Bild` (`BildId`,`Titel`,`Untertitel`,`Erscheinungsjahr`,`Kurzbeschreibung`,`SorteId`,`MalerId`) VALUES
-- ('','','','','','','');
-- INSERT INTO `BildWortZuord` (`BildId`,`Wort`) VALUES
-- ('','');
-- INSERT INTO `VideoWortZuord` (`VideoId`,`Wort`) VALUES
-- ('','');
-- INSERT INTO `EbookTransZuord` (`ISBN`,`TransaktionId`) VALUES
-- ('','');
-- INSERT INTO `HoerbuchTransZuord` (`ISBN`,`TansaktionsId`) VALUES
-- ('','');
-- INSERT INTO `VideoTransZuord` (`VideoId`,`TransaktionsId`) VALUES
-- ('','');
-- INSERT INTO `BildTransZuord` (`BildId`,`TransaktionsId`) VALUES
-- ('','');
-- INSERT INTO `BuchTransZuord` (`ISBN`,`TransaktionsId`) VALUES
-- ('','');
-- INSERT INTO `Maler` (`MalerId`,`PersonenId`,`Beschreibung`) VALUES
-- ('','','');
